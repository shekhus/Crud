...
.then(()=>{
  // create result HTML paragraph to display feedback to user (update the data after smartcontract has changed)
...
.send({from :accounts[0]})
.then(() =>{
  $editResult.innerHTML = `Succesfully updated the name of user ${id} to ${name};`
})
.catch(() =>{
...
.send({from :accounts[0]})
.then(() =>{
  $deleteResult.innerHTML = `User ${id} deleted successfully ! `;
})
.catch(() =>{
  $deleteResult.innerHTML = `Ooops... there was an error while deleting user ${id}!,User does not exists ! `;
});
});
